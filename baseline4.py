...
# -*- coding: utf-8 -*-
"""
Created on Fri Jun  8 11:11:22 2018
...
testdata0=pd.read_csv("/root/bus/dataset/toBePredicted_forUser.csv")
...
#                            print(this_station,time_difference)
    ...
    result=grouped.agg(lambda x: stats.mode(x)[0][0]).reset_index()
    print(result)
    pred_time=''
    for p in range(start_station,end_station+1):
        try:
            x=int(result[result['interval']==p].time)
        except:
            x+=128
        pred_time=pred_time+str(x)+";"
...
predict_table.to_csv("/root/bus/predict_table_lll3.csv",index=0)
